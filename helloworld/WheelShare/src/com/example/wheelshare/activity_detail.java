...
public class activity_detail extends activity_base {
	...
	private void backButtonListener() {
		...
		backkkBttn.setOnClickListener(new View.OnClickListener() {
	...
	private void acceptButtonListener() {
		acceptBttn.setOnClickListener(new View.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				// allow user to accept if seats is not zero.
				// need to display error message
				if (!adInfo.getSeats().equals("0")) {
					Log.i("Accept button", "state 2");
					new UpdateFormTask()
							.execute(requesterUserName, postID, "2");
	...
	private void startPopUpWindow() {
		...
		messageWindowPopup = new PopupWindow(layout, 800, 800, true);
	...
	private void startPaymentPopUpWindow() {
		...
		paymentWindowPopup = new PopupWindow(layout, 800, 800, true);
	...
	private void init() {
		...
		if (activity_result.userInfo != null) {
			...
			if (adInfo.getPosterInfo().getTotalRides() == 0)
				successRateText.setText("No rides");
			else {
				Log.i("RATING SUCCESS", activity_result.userInfo.getUserName()
						+ adInfo.getPosterInfo().getTotalRides());
				Log.i("TOTAL SUCCESS", activity_result.userInfo.getUserName()
						+ adInfo.getPosterInfo().getTotalRides());
				double success = adInfo.getPosterInfo().getSuccessfulRides();
				double total = adInfo.getPosterInfo().getTotalRides();
			...
			if (adInfo.getPosterInfo().getSuccessfulRides() == 0)
				successRateText.setText("No rides");
			else {
				Log.i("RATING SUCCESS1", activity_home.getUserInfo()
						.getUserName()
						+ activity_home.getUserInfo().getSuccessfulRides());
				Log.i("TOTAL SUCCESS1", activity_home.getUserInfo()
						.getUserName() + adInfo.getPosterInfo().getTotalRides());
				double success = adInfo.getPosterInfo().getSuccessfulRides();
