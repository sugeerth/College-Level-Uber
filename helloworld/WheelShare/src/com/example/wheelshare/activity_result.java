...
public class activity_result extends activity_base {
	...
	public void sortBySeatsBttn() {
		...
		sortBySeatsBttn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				Collections.sort(items, new SortByRatingComparator());
				sortByDateBttn.setBackgroundColor(getResources().getColor(
						R.color.BLACK));
				sortByRatingBttn.setBackgroundColor(getResources().getColor(
						R.color.BLACK));
	...
	private boolean fieldErrorCheck() {
		...
		if (dayStr == null || monthStr == null || yearStr == null) {
			...
			int date = Integer.parseInt(yearStr + monthStr);
			Calendar c = Calendar.getInstance();
			int month = c.get(Calendar.MONTH);
			int checkdate = month > 9 ? c.get(Calendar.YEAR) * 100
					+ c.get(Calendar.MONTH) : c.get(Calendar.YEAR) * 10
